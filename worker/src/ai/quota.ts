 ...
 * for signed-in callers, per user instead of per IP.
 */
...
export type QuotaLimits = { global: number; perIp: number; perUser: number }
...
export async function takeQuota(store: QuotaStore, opts: { now: number; ip: string; userId: string | null; limits: QuotaLimits }): Promise<QuotaDecision> {
  ...
  const counts = await store.counts(day, ['global', caller])
