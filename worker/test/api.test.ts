...
describe('meta routes', () => {
  it('describes itself', async () => {
    ...
    expect(await j(res)).toMatchObject({ name: 'BOTC Companion API', mcp: 'http://api.test/mcp', app: env.APP_URL })
    expect((await j(await get('/openapi.json'))).paths['/v1/scripts/drafts']).toBeDefined()
  ...
  it('returns JSON 404s and CORS headers', async () => {
    ...
    expect(await j(res)).toMatchObject({ error: { code: 'not_found' } })
...
describe('catalog routes', () => {
  it('searches characters', async () => {
    const body = await j(await get('/v1/characters?q=imp&lang=zh&limit=3'))
  ...
  it('gets one character with jinxes, bilingual by default', async () => {
    const spy = await j(await get('/v1/characters/spy'))
  ...
  it('searches the rules excerpts', async () => {
    const res = await j(await get('/v1/rules/search?q=nomination%20execution&limit=2'))
    expect(res.items).toHaveLength(2)
    expect((await get('/v1/rules/search')).status).toBe(400)
  })

  it('serves night order, jinxes and editions', async () => {
    const order = await j(await get('/v1/night-order?ids=poisoner,washerwoman,imp&night=first&lang=en'))
    ...
    const jinxes = await j(await get('/v1/jinxes?ids=spy,magician'))
    expect(jinxes.count).toBeGreaterThanOrEqual(1)
    const editions = await j(await get('/v1/editions'))
...
describe('script routes', () => {
  ...
  it('builds a token manifest', async () => {
    const tokens = await j(await get('/v1/scripts/tb/tokens'))
  ...
  it('validates and analyzes', async () => {
    const bad = await j(await post('/v1/scripts/validate', { script: ['imp', 'highpriestess', 'imp'] }))
    ...
    const analyzed = await j(await post('/v1/scripts/analyze', { slug: 'tb' }))
  ...
  it('refuses drafts with errors', async () => {
    ...
    expect((await j(res)).validation.ok).toBe(false)
