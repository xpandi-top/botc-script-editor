...
import { evalDrafts, formatSummary, summarize } from './feedbackReport.mjs'
...
if (!file) fs.writeFileSync(path.join(outDir, `feedback-${stamp}.jsonl`), rows.map((r) => JSON.stringify(r)).join('\n') + (rows.length ? '\n' : ''))
...
console.log(`\n${drafts.length} draft eval cases → .feedback/eval-drafts-${stamp}.json${file ? '' : `\nAll rows → .feedback/feedback-${stamp}.jsonl`}`)
