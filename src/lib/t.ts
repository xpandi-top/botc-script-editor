...
export type UiKey =
  ...
  | 'note_this_is_a_new_character_not_yet_saved_to_the_database'
  ...
  | 'alive_2' | 'skills_used' | 'events' | 'full_log' | 'recent_scripts' | 'game_analytics' | 'general'
  | 'exile_2'
  // ScriptsTab keys
  ...
  | 'this_link_opens_this_script_directly_link_never_expires'
  | 'this_link_contains_a_full_copy_of_the_script_valid_for_24_ho'
  // Tutorial keys
  ...
  | 'switch_between_the_warm_parchment_light_theme_and_the_dark_c'
  | 'scales_all_ui_text_spacing_and_controls_uniformly'
  | 'chinese_font_applies_to_both_body_text_and_titles'
  | 'font_settings_are_saved_locally_and_persist_across_reloads_g'
  | 'export_a_full_backup_containing_scripts_custom_characters_re'
  ...
  // LeftScriptPanel extra
  | 'records'
  // Remaining keys from bulk migration
  // Final remaining keys
  | 'add_folder' | 'add_script' | 'autogenerated_from_script_date_if_blank' | 'changelog' | 'character'
  | 'character_team_used_in_analytics_blank_names_autouse_seat_nu' | 'characters_played' | 'chat'
  ...
  | 'new_game_record' | 'next_day_2' | 'nominate' | 'other_notes' | 'player_list' | 'previous_day'
  | 'quick_edit' | 'show_wake_order' | 'skills' | 'st_games' | 'st_name' | 'st_win' | 'survey_st' | 'who_ran_this_game'
  // More missing keys from remaining errors
  | '1_col' | '2_col' | 'active_players' | 'alignment' | 'alphabetical' | 'balanced_win_rates' | 'bluffs' | 'body' | 'card_outline' | 'chinese_2' | 'chip' | 'columns' | 'common_characters' | 'compact' | 'configure_tags_to_see_preview' | 'consider_harder_evil_roles' | 'consider_more_goodfavoured_scripts' | 'demon_2' | 'english_font' | 'evil_games' | 'evil_rate' | 'evil_w' | 'evildominant' | 'exec' | 'exporting' | 'games' | 'good_games' | 'good_w' | 'gooddominant' | 'hide_menu' | 'icon_outer_circle' | 'icons' | 'in_scripts' | 'inline' | 'insert_character_after_this' | 'last_10_games' | 'layout' | 'left' | 'line' | 'min' | 'minion_2' | 'mixed' | 'most_played' | 'n' | 'no_character_data_add_character_assignments_to_records' | 'noms' | 'normal' | 'outsider_2' | 'page' | 'page_1_end' | 'perday_averages' | 'player_comparison' | 'players_who_played_this' | 'preview_actual_print_may_differ_slightly' | 'print_preview' | 'print_studio' | 'recent_appearances' | 'right' | 's' | 'script_pdf' | 'search_character' | 'section' | 'section_style' | 'select_characters_on_the_left' | 'separate' | 'show_author' | 'show_menu' | 'side' | 'spacing' | 'spacious' | 'st' | 'str_en' | 'switch_to_script_print_preview' | 'times_used_as_demon_bluff' | 'toggle_language' | 'tokens' | 'typography' | 'unique_chars' | 'used_as_demon_bluff' | 'wake_order' | 'win_rate' | 'zh'
  | '1_wake_first' | 'ability' | 'ability_size' | 'add_marker' | 'add_this_uri_to_google_cloud_console_authorized_redirect_uri' | 'ai_assistant' | 'all_scripts' | 'anon' | 'anonymous' | 'apply_start_game' | 'arc' | 'ask_the_storyteller_for_a_new_link' | 'attributes' | 'autoapply_fills' | 'avg_days' | 'avg_min' | 'avg_players' | 'avg_ratings' | 'background' | 'balanced' | 'bg_color' | 'black_white' | 'blood_on_the_clocktower_deal' | 'border' | 'both' | 'bottom' | 'bright' | 'center' | 'change' | 'change_image' | 'character_markers' | 'characters' | 'chars' | 'check_network_or_reconnect' | 'chinese_body' | 'circle' | 'claimed' | 'clear_chat' | 'click_a_skill_card_to_run_it' | 'click_to_rename' | 'close_deal_no_more_claims' | 'closed' | 'color' | 'community' | 'compare' | 'configure_your_own_google_oauth2_credentials' | 'confirm_delete' | 'connect_google_drive_to_automatically_sync_scripts_custom_ch' | 'connection_failed' | 'contain' | 'copied' | 'copy_player_link' | 'copy_to_diy' | 'could_not_claim_that_card_please_try_again' | 'cover' | 'create' | 'custom_tags' | 'data_stored_in_your_private_google_drive_appdatafolder_only_' | 'data_stored_privately_in_your_google_drive' | 'date' | 'deal_dashboard' | 'dealing_closed' | 'delete_folder' | 'delete_selected' | 'delete_this_record' | 'demon_info' | 'diy' | 'e' | 'empty_folder' | 'en_font' | 'enable_watermark' | 'english_body' | 'english_title' | 'ensure_this_exact_redirect_uri_is_registered_in_cloud_consol' | 'error' | 'evil_2' | 'evil_rate_of_games_played_as_evil' | 'exp' | 'export_markdown' | 'export_selected' | 'filtered' | 'first' | 'first_night_en' | 'first_night_reminder_en' | 'first_night_reminder_zh' | 'first_night_zh' | 'fit' | 'folder_name' | 'from' | 'fun_evil' | 'fun_good' | 'g' | 'general_ai_assistant' | 'global_tokens_all_seats' | 'hex' | 'hide_abilities' | 'hide_characters' | 'hide_reminders' | 'icon' | 'icon_size' | 'image' | 'insights' | 'label' | 'label_optional' | 'link_expired' | 'loading' | 'markers' | 'minion_info' | 'mode' | 'more' | 'most_played_characters' | 'move_to_folder' | 'mvp_count' | 'name' | 'name_size' | 'new_folder' | 'new_record' | 'night_reminders' | 'no_community_scripts' | 'no_custom_scripts_yet_use_the_buttons_above_to_get_started' | 'no_fills_applied_yet' | 'no_matches' | 'no_night_reminders_for_this_character' | 'no_night_wake' | 'no_night_wakeup' | 'no_player_data_add_player_names_to_game_records' | 'no_records_match_the_current_filter' | 'no_records_yet_complete_a_game_or_add_records_manually_to_se' | 'no_script_data' | 'no_wakeup' | 'number_size' | 'numbers' | 'oauth_credentials_are_preconfigured_just_click_connect_googl' | 'official' | 'open_cloud_console' | 'other' | 'other_nights_en' | 'other_nights_reminder_en' | 'other_nights_reminder_zh' | 'other_nights_zh' | 'otherseat_tokens' | 'output' | 'p' | 'page_size' | 'pick_your_character_card' | 'player_name' | 'players' | 'position' | 'qty' | 'record' | 'reminder_tokens_2' | 'remove_from_folder' | 'rename_folder' | 'replay' | 'reset_filters' | 'retry' | 'save_both_client_id_and_client_secret_above_first' | 'saved_keep_your_character_secret' | 'saved_register_this_redirect_uri_in_cloud_console' | 'script_summary' | 'scripts' | 'search_characters_2' | 'search_idnamedescription' | 'search_scripts_author_characters' | 'seat' | 'seat_optional' | 'selected' | 'set_claimed' | 'set_unclaimed' | 'setup_indicators' | 'shape' | 'shape_size' | 'show_abilities' | 'show_characters' | 'show_reminders' | 'skills_for_other_contexts' | 'square' | 'st_games_run' | 'storytellers' | 'straight' | 'stretch' | 'style' | 'switch_to_list_view' | 'tag_type' | 'tap_one_card_to_flip_it_others_will_lock' | 'tap_to_flip' | 'text' | 'that_card_was_already_claimed_pick_another_card' | 'the_storyteller_has_closed_this_deal_session' | 'thinking' | 'to' | 'tokens_available_on_every_seat_regardless_of_assignment' | 'tokens_this_character_places_on_other_players' | 'top_5_players' | 'total_games' | 'type_a_message_enter_to_send' | 'uncategorized' | 'unclaimed' | 'undo' | 'upload_image' | 'use_skill_chips_above_or_type_a_message' | 'view_cards' | 'w' | 'wake_first' | 'wake_order_indicators' | 'watermark_optional' | 'watermark_text' | 'win_balance' | 'your_character' | 'your_name_optional' | 'zh_font'
  ...
  | 'no_events_found_for_this_player' | 'vote_2' | 'skill' | 'tag' | 'state'
  ...
  // StorytellerSub — additional keys used in @ts-nocheck components
  | 'add_all' | 'alice_bob_charlie' | 'all_data_for_this_day_events_votes_skill_log_will_be_permane'
