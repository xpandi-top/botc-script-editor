/**
 * Player identity history: each seat's starting role/alignment and every
 * change during the game, and the per-player summaries saved in records.
 * The character catalog is injected as a TeamLookup; src/utils/playerIdentity.ts
 * binds it to the app catalog.
 */
import type { DayState, GameRecord, IdentityHistory, IdentityState, PlayerSummary, StorytellerSeat } from '../types/game'
import { seatAlignmentWith, type TeamLookup } from './alignment'

export type IdentityBasis = 'initial' | 'final'
export const identityOf = (getTeam: TeamLookup, seat: StorytellerSeat): IdentityState => ({ characterId: seat.characterId, team: seatAlignmentWith(getTeam, seat) })
const differs = (a: IdentityState, b: IdentityState) => a.characterId !== b.characterId || a.team !== b.team

export function createIdentityHistory(seats: StorytellerSeat[], getTeam: TeamLookup, complete = true): IdentityHistory {
  return { complete, initial: seats.map(s => ({ seat: s.seat, ...identityOf(getTeam, s) })), changes: [] }
}

/** Called inside the state updater; undo restores history together with the seats. */
export function trackIdentityUpdates(previous: DayState[], next: DayState[], getTeam: TeamLookup, now: number): DayState[] {
  const identity = (seat: StorytellerSeat) => identityOf(getTeam, seat)
  return next.map(day => {
    ...
    const changes = day.seats.flatMap(seat => {
      ...
      return old && differs(identity(old), identity(seat))
        ? [{ seat: seat.seat, at: now, phase: day.phase, from: identity(old), to: identity(seat) }] : []
    })
    const added = day.seats.filter(s => !before.seats.some(old => old.seat === s.seat))
    if (!changes.length && !added.length) return day
    const history = before.identityHistory ?? createIdentityHistory(before.seats, getTeam, false)
...
export function summarizeIdentities(days: DayState[], getTeam: TeamLookup): PlayerSummary[] {
  const identity = (seat: StorytellerSeat) => identityOf(getTeam, seat)
...
export function recordPlayers(record: GameRecord, getTeam: TeamLookup): PlayerSummary[] {
  const derived = record.savedDays?.length ? summarizeIdentities(record.savedDays, getTeam) : []
