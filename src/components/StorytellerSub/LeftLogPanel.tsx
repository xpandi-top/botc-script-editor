...
function phaseLabel(phase: string, text: any): string {
  ...
  return phase // legacy or unknown — show as-is
}
...
export function LeftLogPanel({ ctx }: { ctx: any }) {
  ...
  // Group entries by day for structured display
  const grouped = React.useMemo(() => {
    const map = new Map<number, typeof aggregatedLog>()
    ...
    <aside className={`storyteller-side-panel storyteller-side-panel--left${showLogPanel ? ' storyteller-side-panel--open' : ''}`}>
      ...
      <div className="storyteller-log-filters">
        {(['all', 'public', 'st-only'] as const).map((v) => (
          <button
            key={v}
            className={`secondary-button secondary-button--small${logFilter.visibility === v ? ' tab-button--active' : ''}`}
            onClick={() => setLogFilter((p: any) => ({ ...p, visibility: v }))}
            type="button"
          >
        ...
        <button className="secondary-button secondary-button--small" onClick={() => setLogFilter((p: any) => ({ ...p, sortAsc: !p.sortAsc }))} type="button">
          {logFilter.sortAsc ? '↑' : '↓'}
        </button>
      </div>
