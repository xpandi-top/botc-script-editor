...
import {
  ...
  TextField, IconButton, Button, Checkbox, Chip, FormGroup, FormControlLabel as FCL, Tooltip,
} from '@mui/material'
...
export function TokenOptionsPanel({ opts, onChange, scriptCharacters, language, pinnedRevisions }: Props) {
  ...
  const ptSlider = (text: string, key: 'nameFontSize' | 'abilityFontSize' | 'numberFontSize', min: number, max: number) => (
    <Box sx={{ mb: 1 }}>
      ...
      <Slider value={opts[key]} min={min} max={max} step={0.5}
        aria-label={text} valueLabelDisplay="auto" valueLabelFormat={(value) => `${value} pt`}
  ...
  const fontSelect = (labelStr: string, key: 'fontKeyEn' | 'fontKeyZh') => (
    <FormControl size="small" fullWidth sx={{ mb: 1 }}>
      <InputLabel id={`token-${key}-label`}>{labelStr}</InputLabel>
      <Select value={opts[key]} labelId={`token-${key}-label`} label={labelStr}
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, px: 2, py: 1.5 }}>
      ...
      <Box>
        {label(t('mode'))}
        <ToggleButtonGroup value={opts.mode} exclusive size="small" fullWidth aria-label={t('mode')}
...
{/* ── Character selection ── */}
      {opts.mode === 'characters' && (
        <Box>
          ...
          {/* Search */}
          <TextField
            fullWidth size="small" placeholder={t('search_characters')}
            value={search} onChange={(e) => setSearch(e.target.value)}
            slotProps={{ htmlInput: { 'aria-label': t('search_characters') }, input: {
              startAdornment: <SearchIcon sx={{ fontSize: 18, mr: 0.5, color: 'text.secondary' }} />,
              endAdornment: search ? <IconButton size="small" aria-label={t('clear')} onClick={() => setSearch('')}><ClearIcon fontSize="small" /></IconButton> : null,
            } }}
            sx={{ mb: 1 }}
          />
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mb: 1 }}>
            {TEAM_FILTERS.map(f => (
              <Chip
                key={f.value}
                size="small" label={zh ? f.zh : f.en}
                color={teamFilters.has(f.value) ? 'primary' : 'default'}
                variant={teamFilters.has(f.value) ? 'filled' : 'outlined'}
                aria-pressed={teamFilters.has(f.value)}
          ...
          <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 0.5 }}>
            {tpl('pack_n_chars_selected', filteredCharacters.length, opts.selectedCharacterIds.length)}
          </Typography>
          <Box sx={{ display: 'flex', gap: 0.5, mb: 0.5 }}>
            <Button size="small" variant="text" disabled={filteredIds.length === 0}
              onClick={() => set('selectedCharacterIds', filteredIds)}>
              {search || !teamFilters.has('all') ? t('print_select_matches') : t('print_select_all')}
            </Button>
            <Button size="small" variant="text" disabled={opts.selectedCharacterIds.length === 0}
              onClick={() => set('selectedCharacterIds', [])}>
              {t('print_clear_selection')}
            </Button>
          </Box>

          {filteredCharacters.length === 0 && (
            <Box sx={{ textAlign: 'center', py: 2, bgcolor: 'action.hover', borderRadius: 1 }}>
              <Typography variant="body2" color="text.secondary">{t('no_matches')}</Typography>
              <Button size="small" onClick={() => { setSearch(''); setTeamFilters(new Set(['all'])) }}>{t('reset_filters')}</Button>
            </Box>
          )}
          
          <FormGroup sx={{ flexWrap: 'nowrap', maxHeight: 260, overflowY: 'auto', pl: 0.5, border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
              ...
              <FCL
                ...
                label={
                  <Typography variant="caption">
                    {getDisplayName(c.id, language)} · {t(c.team)}
                  </Typography>
                }
              />
            ))}
          ...
          {opts.tagMode === 'numbers' && (
            <Box>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <input type="color" aria-label={t('bg_color')} value={opts.numberBgColor}
            ...
            <Box>
              {opts.markers.map((m) => (
                <Box key={m.id} sx={{ display: 'flex', gap: 0.5, alignItems: 'center', mb: 0.75, flexWrap: 'wrap' }}>
                  ...
                  <input type="color" aria-label={`${t('bg_color')} ${m.label || m.icon}`} value={m.bgColor}
                  ...
                  <Tooltip title={t('delete')}>
                    <IconButton size="small" aria-label={`${t('delete')} ${m.label || m.icon}`} onClick={() => removeMarker(m.id)}>
      ...
      {/* Token shape + size */}
      <PrintMenuSection title={t('shape_size')}>
        <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 0.5 }}>
          {t('shape')}
        </Typography>
        ...
        {opts.shape !== 'rectangle' && (
          <Box sx={{ mb: 1 }}>
            ...
            <Slider aria-label={tpl('diameter_mm', opts.diameterMm)} valueLabelDisplay="auto" value={opts.diameterMm} min={10} max={80} step={1}
        ...
        {opts.shape === 'rectangle' && (
          <Box>
            <Box sx={{ mb: 1 }}>
              ...
              <Slider aria-label={zh ? '宽度' : 'Width'} valueLabelDisplay="auto" value={opts.rectWidthMm} min={40} max={150} step={1}
            ...
            <Box sx={{ mb: 1 }}>
              ...
              <Slider aria-label={zh ? '高度' : 'Height'} valueLabelDisplay="auto" value={opts.rectHeightMm} min={15} max={80} step={1}
        ...
        <Box>
          ...
          <Slider aria-label={tpl('gap_mm', opts.gapMm)} valueLabelDisplay="auto" value={opts.gapMm} min={0} max={10} step={0.5}
        ...
        <Box>
          ...
          <Slider aria-label={tpl('margin_mm', opts.marginMm)} valueLabelDisplay="auto" value={opts.marginMm} min={0} max={30} step={1}
        ...
        <PrintMenuSection title={t('text')}>
          <Box>
            ...
            <Box sx={{ mb: 1 }}>
              ...
              <Slider aria-label={t('icon_size')} valueLabelDisplay="auto" valueLabelFormat={(value) => `${Math.round(value * 100)}%`} value={opts.iconSizeRatio} min={0.4} max={2.0} step={0.05}
      ...
      {/* Background */}
      <PrintMenuSection title={t('print_section_look')}>
        <Typography variant="caption" color="text.secondary">{t('background')}</Typography>
        <ToggleButtonGroup value={opts.bgType} exclusive size="small" aria-label={t('background')}
        ...
        {opts.bgType === 'color' && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <input type="color" aria-label={t('bg_color')} value={opts.bgColor}
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1 }}>
          ...
          <input type="color" aria-label={`${t('border')} · ${t('color')}`} value={opts.borderColor}
            onChange={(e) => set('borderColor', e.target.value)}
            style={{ width: 32, height: 24, border: 'none', padding: 0, cursor: 'pointer' }} />
        </Box>
        <Slider aria-label={t('border')} valueLabelDisplay="auto" value={opts.borderWidth} min={0} max={6} step={0.5}
          onChange={(_, v) => set('borderWidth', v as number)}
          size="small" />
      </PrintMenuSection>

      {/* Output */}
      <PrintMenuSection title={t('output')}>
        <FormControl size="small" fullWidth sx={{ mb: 1 }}>
          <InputLabel id="token-page-size-label">{t('page_size')}</InputLabel>
          <Select value={opts.pageSize} labelId="token-page-size-label" label={t('page_size')}
      ...
      {/* Watermark */}
      <PrintMenuSection title={t('watermark_optional')} defaultExpanded={opts.watermarkEnabled}>
        <FormControlLabel
        ...
        {opts.watermarkEnabled && (
          <Box sx={{ mt: 1, pl: 1, borderLeft: '2px solid', borderColor: 'divider' }}>
            ...
            {opts.watermark.type === 'text' ? (
              <Box>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                  <Typography variant="caption" color="text.secondary">{t('color')}</Typography>
                  <input type="color" aria-label={`${t('watermark_optional')} · ${t('color')}`} value={opts.watermark.color}
                ...
                <Box sx={{ mb: 1 }}>
                  ...
                  <Slider aria-label={tpl('watermark_size_pt', opts.watermark.fontSize)} valueLabelDisplay="auto" value={opts.watermark.fontSize} min={4} max={16} step={0.5} size="small"
            ...
            <Box>
              ...
              <Slider aria-label={tpl('watermark_opacity_pct', Math.round(opts.watermark.opacity * 100))} valueLabelDisplay="auto" valueLabelFormat={(value) => `${Math.round(value * 100)}%`} value={opts.watermark.opacity} min={0.05} max={0.8} step={0.05} size="small"
