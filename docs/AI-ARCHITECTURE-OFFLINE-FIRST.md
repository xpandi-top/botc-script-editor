...
日期：2026-09-23 初稿；2026-09-24 修订（§0，实施与实测结果见 §15–§17）。

初稿基于当时的工作区（Groq、上下文预算、奥德赛检索修复）。§0 是修订后的决策；§1–§14 保留初稿的评审与计划，已解决的问题在 §2 中标注。

## 0. 修订后的决策（2026-09-24）

1. **两个主模式，自带 Key 只是可选项。** 设置页首先给出“在线 · BOTC 免费”（托管，无需 key，每日限额）和“本地 · 离线”（本地资料 + 规则程序；可选下载本机模型）。“自带 Key”（Groq / OpenRouter / Gemini）放在第三个标签，标注“可选”。构建配置了 API 时默认在线，否则默认本地；已填 key 的老用户不变。
2. **程序计数，模型解释，程序校验。** 事实（能力原文、数量、名单、相克、夜序）、规则数字（处决票数、按人数的配置、设置修正）、开局配置、剧本角色池、局面胜负条件都由程序计算，作为“程序计算的规则事实”放进每个运行时的提示词；模型回答里的开局 / 剧本名单再由程序校验，不合法就附上程序生成的合法方案并说明原因（§15）。
3. **离线不依赖模型。** 没有模型、没有网络、在线额度用完时，面板直接给出本地资料回答（评测 22/22 通过，§17）；本机模型（WebLLM）只是在此之上增加自然语言解释。
4. **托管模型：GLM-4.7-flash（关闭思考）。** 免费档可用的四个带工具调用的模型实测对比见 §17：没有一个能可靠完成多约束生成和局势推理，GLM 最便宜、最快、在有证据时事实题可靠；难题交给程序（第 2 点）。Gemma-4-26B 推理较好但慢且不稳定，留作以后的“深度模式”候选。
5. **上下文有界。** 页面的关键状态（剧本 id 名单、对局座位与存活人数）放在上下文首段并始终保留；其余资料按问题相关度选取。各运行时的输入预算、工具输出上限和每日 neurons 预算见 §16。
6. **规则文本经过核对。** 核心规则只写本地官方资料能支持的内容（`src/core/ai/rules.ts`，逐条有回归测试），修正了初稿指出的“超过半数”等错误（§2）。
7. **向量在服务端、随目录自动更新。** 角色向量与查询向量都由 Workers AI 的同一模型生成，按文本哈希增量更新；每次部署后冒烟测试触发更新并要求无过期（§16）。
...
| 优先级 | 已确认的实现 | 影响 | 建议 |
|---|---|---|---|
| P0 ✅ 已修 | `src/lib/ai/prompts.ts:118/202` 把普通处决票数门槛写成“超过半数”，并把核心规则标为高优先级 | 即使检索、模型都正常，也可能被错误参考资料引导 | 核对规则来源，移除未审核的高优先级规则摘要；程序计算与版本化规则测试共同约束 |
| P0 ✅ 已修 | `src/lib/aiSettings.ts:93` 每次读取都让环境 provider 覆盖用户选择，迁移也会重置 provider | UI 切换后可能重新使用旧服务商；环境 key 也覆盖用户输入 | 明确配置优先级：会话选择 > 用户配置 > 安装默认；环境变量只用于默认值，不能反复改写用户配置 |
| P0 ✅ 已修 | `src/components/AiPanel/ChatTab.tsx:289` 无 key 禁用发送；`isAiAvailable` 也只判断 key | 本地查询和本地模型无法独立使用 | 用任务能力状态代替 `hasApiKey`；按钮、Enter、技能快捷入口使用同一检查 |
| P0 ✅ 已修 | `worker/src/mcp.ts:87–92` 搜索默认 limit=20、上限100，返回 `count: items.length` | 119 个角色可能呈现为20条结果；工具缺少总数与下一页语义 | 增加总数、分页及独立 aggregate 工具。**这是一条潜在错误路径，并非声称前次聊天调用过 MCP** |
...
| P1（向量部分 ✅） | `src/lib/botcSearch.ts:105` 主要索引英文能力，按空格分词；查询向量仍需 Gemini | 中文召回弱；预生成向量不等于运行时离线向量检索 | 双语词法索引先补齐；可选本地 multilingual embedding，统一文档与 query 的模型和处理参数 |
| P1 | PWA 预缓存规则未包含通用 JSON，runtimeCaching 只覆盖部分资源 | Wiki/embeddings 等首次使用未必能在断网下取得 | 明确“离线资料包”安装、完整性、版本、缓存状态，实际断网启动验收 |
| P1（部分 ✅） | 前端聊天不执行现有 MCP 工具；Worker 工具已复用部分 core，但很多包装与环境绑定 | 同一事实/校验功能出现不同实现和行为 | 先提取共享领域服务和工具契约，再提供本地与 MCP 两种入口 |
...
## 15. 实施：两个主模式、程序事实与答案校验

| 部分 | 实现 | 位置 |
|---|---|---|
| 在线 · BOTC 免费 | 托管运行时调 API Worker 的 `/v1/ai/chat`：GLM-4.7-flash + 本项目 MCP 工具（进程内连接），key 在服务端；每日限额与 neurons 预算；失败时给出原因并附上本地资料回答 | `src/lib/ai/runtime/hosted.ts`、`worker/src/ai/` |
...
| ✅ | D：WebLLM 本地模型已在 WebGPU 上实测（见 §18）；精确问题由程序回答不经模型；缓存完整性检测与自动续载；离线冷启动有端到端测试 |
...
| 项 | 取值 | 说明 |
|---|---|---|
| 提示词输入预算（估算 token） | Groq 5500 · 托管 12000 · WebLLM 3000 | 估算对中文偏保守（每字按 3）；托管实测每题输入约 3k–8k token |
| 页面上下文 | 首段始终保留；其余按问题相关度选段 | 首段过大（超过预算 80%）时退回逐行选取 |
| 服务端工具 | 每轮最多 4 次调用、最多 5 轮，工具输出累计上限 16,000 字符 | 超过即要求模型作答；空回答时不带工具再问一次 |
| 每日限额 | 全站 300 次、每 IP 30 次、登录用户 100 次、neurons 8000（免费档共 10,000） | 实测 GLM 一题约 20 neurons（一轮）到 90（多轮工具） |
| 向量 | Workers AI `bge-m3`，文本 = 中英名称 + 中英能力；按 (模型, 角色) 存 D1，附文本哈希 | 每个 isolate 首次相似查询时补算哈希不符 / 新增的角色、删除已移除的角色；换模型即全量重算 |
| 部署时更新向量 | `npm run deploy` 后自动冒烟测试（`postdeploy`）：语义检索触发更新，`/v1/ai/status` 必须 stale = 0；CI 的 `deploy-worker.yml` 在目录数据变化时自动部署 | 查询向量与文档向量同一模型；前端不再有静态向量文件 |
...
- 评分器本身有单测（`aiEvalGraders.test.ts`、`aiAnswerCheck.test.ts`）。

**结果（2026-09-24，本地 `wrangler dev` + 真实 Workers AI）。**

| 轮次 | 改动 | GLM-4.7-flash 通过 |
|---|---|---|
| 1 | 修正核心规则后首次完整运行 | 12 / 22 |
| 2 | 工具指引“先用上下文”；引用能力原文时给出双语官方文本 | 13 / 22 |
| 3 | 页面状态始终保留；程序计算票数 / 配置 / 开局 / 剧本 / 局面 | 15 / 22 |
| 4 | 答案校验（之前失败的 7 题复测） | 6 / 7（剩余 1 题为模型空回答，已加重试与本地回退） |
| — | 本地资料回答（无模型） | 22 / 22 |

难题子集（9 题：翻译、醉酒、两道开局、两道剧本、三道局势）的模型对比，均未加答案校验：

| 模型（免费档，带工具调用） | 通过 | neurons | 平均耗时 | 备注 |
|---|---|---|---|---|
| `@cf/google/gemma-4-26b-a4b-it` | 4 / 9 | 535 | 31 s | 推理最好；4 题服务端失败 |
| `@cf/zai-org/glm-4.7-flash` | 2 / 9 | 381 | 16 s | 最便宜；证据在场时事实可靠 |
| `@cf/meta/llama-4-scout-17b-16e-instruct` | 1 / 9 | 1077 | 3 s | 快，但每题约 120 neurons |
| `@cf/qwen/qwen3-30b-a3b-fp8`（思考开） | 1 / 9 | 629 | 8 s | 常不按要求输出名单 |

结论：免费档模型都不能单独可靠地完成多约束生成和局势推理，因此采用“程序事实 + 答案校验 + 本地回退”；GLM 作为默认托管模型。
...
## 18. 本地模型（WebGPU）实测与离线路由

在本机 Chrome（Apple M 系列，Metal，shader-f16）上实测：

| 模型 | 下载大小 | 首次下载+加载 | 从缓存加载 | 解释类问题 | 质量 |
|---|---|---|---|---|---|
| Qwen3 1.7B | ≈ 0.95 GB | ≈ 33 s | 3–9 s | 20–26 s | 解释、比较正确；票数答成“5票”（应为 3）；剧本推荐超出 4K 窗口 |
| Qwen3 0.6B | ≈ 0.38 GB | ≈ 9–14 s | ≈ 6 s | ≈ 6–15 s | 解释、比较基本正确，要点较泛 |

结论与改动：

- **精确问题不经模型。** 数量、票数、开局配置、能力原文、剧本的相克 / 夜晚顺序 / “哪些角色会醉酒或中毒”、角色包统计、剧本推荐，本地模式下直接给程序答案（即时、准确）；模型只接解释、建议、比较类问题（`localAnswer.definitive`）。
- **本地回答只给相关内容：** 页面对局状态只在问到当前局面时出现；有程序答案时不附规则段落；规则段落按相对得分筛选，角色问题只附提到该规则词的段落；wiki 摘录只在没有精确答案时出现。
- **提示词预算按 Qwen 分词估算**（中文约 1.3 token/字，旧估算按 3 计，4K 窗口只能放约 1000 字）：本地模型现在拿到程序事实、目录原文、最相关的两段核心规则和 wiki 摘录，回答分 3–5 条要点；单行“1. … 2. …”显示为列表。
- **离线：** 选了任何在线模型但浏览器离线时，直接给本地资料，不发请求；网络错误显示“无法连接在线 AI”。
