...
## 15. 托管运行时（BOTC 免费 AI）与实施进度

2026-09-23 与 `feat/hosted-ai`（`docs/ARCHITECTURE-API.md` §7.5）合并后的定位：

| 项 | 做法 | 与本方案的关系 |
|---|---|---|
| 托管运行时 `botc` | `src/lib/ai/runtime/hosted.ts` 调 API Worker 的 `/v1/ai/chat`；模型在服务端（Workers AI，GLM-4.7-flash），key 不进浏览器；每日限额（全站 / 每 IP / 登录用户） | §4 “可选在线综合”的一种：明确零价端点（对用户），§8 的额度与 429 降级提示已实现；免费额度用尽时提示改用本地模型或自己的 key，不会转收费 |
| 默认值 | 构建配置了 `VITE_API_URL` 时，首次使用默认 `botc`；没有 key 的 key 型 provider 也改用它；已填 key、WebLLM、带 key 的环境默认保持不变 | 与 §1 “新用户默认本地优先、在线未启用”不同：按产品要求（开箱可用、不填 token）选择在线默认。设置页写明问题与页面资料会发到服务器；完全离线请选 WebLLM。“严格离线 / 在线增强授权级别”仍按 §7 待做 |
| 证据与工具 | 前端照旧先做本地检索与证据拼装（`prepareSystemPrompt`，托管运行时使用更大的输入预算）；服务端再通过进程内 MCP 连接调用共享 core 的只读工具 | 符合 §9：工具逻辑在 `src/core`，MCP 只是对外包装；服务端工具调用不经浏览器往返 |
| 向量 | 角色向量与查询向量都在服务端用同一个模型（bge-m3）生成，目录变更后按文本哈希自动重算；前端 `findSimilar` 走 API，WebLLM / 无 API / 失败时回退本地 TF-IDF | 解决 §6 “Gemini 向量不能与其他模型的 query 向量混用”：不再有静态 Gemini 向量 |
| 诊断 | 托管回答下方显示本次调用的工具与今日剩余次数 | §7 “诊断”的第一步 |

实施进度（对照 §12）：

| 状态 | 项 |
|---|---|
| ✅ | A：设置优先级修复（保存的选择优先、保留用户 key）；无 key 也可问答（托管运行时）；MCP 总数 / 分页（`totalMatches`、`nextCursor`、`list_editions`） |
| ✅ | D（部分）：WebLLM 运行时（Qwen3 1.7B / 0.6B，显式下载、取消、释放） |
| ✅ | F（部分）：托管在线运行时、每日限额与 429 / 额度用尽 / 服务不可用的降级提示 |
| ⬜ | A：无模型时的确定性回答（计数、名单、能力原文直接由程序回答）；核验并替换核心规则摘要 |
| ⬜ | B / C：统一 QueryPlan / EvidenceBundle、有效目录快照、离线资料包与严格离线门禁 |
| ⬜ | F：回答与证据的自动核对（实测 GLM-4.7-flash 仍可能编造能力，已用工具指引约束，但需要程序校验） |
