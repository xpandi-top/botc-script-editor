...
- [x] **A4 · 夜序数值化** — ✅ 已完成，而且**顺带修了一个真实的排序 bug**。
      `night-order.json` 加了两个字段，**数组本身没动，10 个消费方一行都不用改**：
      - `order`：每个条目一个位置数值，按 ×10 间隔，新角色可以插进两个已有条目之间；
      - `source_order`：角色包自己公布的夜序数值（奥德赛的「夜序数值」），
        以后同步时可以直接按数值比大小定位，不用再靠「前位角色」锚点。
      catalog 加了 `getNightOrderValue` / `getNightSourceValue` / `nightOrderFromValues`。
      两份数据会漂移，所以 `nightOrderValues.test.ts`（11 条）钉死：每个条目都有值、没有孤儿值、
      **按值排序必须还原出原数组**、值唯一且留有间隙、数组顺序与包公布的数值一致。

      🐛 **导入时的排序 bug（已修）**：`emit.py` 把同一个「前位角色」下的多个角色
      **各自插到锚点正后方**，结果整组顺序被反转。其他夜有 9 处受影响，
      比如 `versifier(1440) → sphinx(1430) → cyclops(1420) → crosswing(1410)` 完全倒序。
      重建后首夜 0 处倒序、其他夜只剩 1 处（见下），条目总数不变（120 / 189）。
      `emit.py` 已改成按组累加偏移插入，重新同步不会再犯。

      ⚠️ 剩下的 1 处倒序是数据本身的分歧，没有强行改：
      `corruptus`（锚点 奥赫 ojo）排在 `skeleton_king`（锚点 堤丰之首 lord_of_typhon）之前，
      但奥德赛给的数值是 6210 > 5710。原因是本仓库里 ojo 排在 lord_of_typhon 之前，
      和奥德赛编号时假设的顺序相反。两者都是恶魔，一局只有一个恶魔在场，**对实际游戏没有影响**，
      所以保留了角色包公布的锚点，测试里作为具名例外记录。
...
- [x] **B2 · 能力文本抓取准确性抽查** — ✅ 已完成，**不是抽查，是 119 个全量核对**。
      `scripts/odyssey/verify.py` 重新拉一遍百科，用**和导入管线不同的解析路径**取能力文本
      （导入是整页转文本后按标题行切段；校验是直接在 lake HTML 里按 `角色能力`→`角色信息`
      的位置切片），再和仓库比对。两条独立路径一致才算数。
      结果：**119/119 一致，提取零错误**。唯一一处差异是上游在 2026-08-31 改了能力（见下）。
      跑法（需要联网）：`python3 scripts/odyssey/verify.py`，有差异时退出码 1。
      工作目录 `.odyssey-verify/` 已加进 `.gitignore`。

      **顺带发现并同步了一次上游改动**：对比两次抓取的 `content_updated_at`，131 篇里有 3 篇变了，
      其中 `073 骚客 Versifier` 是能力调整（百科自己的「角色调整记录」也记了 2026.8.31 这条）：
      「首次有玩家吟诵」→「首次有**存活**玩家吟诵」。
      按项目规则**新增 revision `v2026-08`**（`v1` 原文保留），没有覆盖；
      配套的运作方式、范例、规则细节、提示标记也一并刷新了 almanac 条目。
      另外两篇变动是「角色调整记录」和「奥德赛杯策划案」，都不是角色数据。
...
- [x] **C4 · 官方角色夜序数值** — ❌ **查证后关闭：这份数据不存在，A4 不该依赖它**（2026-08-31）。
      找过的源：`script.bloodontheclocktower.com/data/roles.json`（404）、`clocktower.online/roles.json`（404）、
      `bra1n/townsquare` 的 `src/roles.json`（唯一能拉到的，130 个角色，带 `firstNight`/`otherNight` 整数）。
      townsquare 这份用不了，两个独立原因：
      1. **覆盖不够** —— 本仓库夜序表里有 135 个 id 它根本没有。首夜 116 个角色只有 50 个有数值，
         其他夜 187 个只有 70 个。缺的里面有 45 个是华灯初上/山雨欲来两个中文自制包，
         **这两个包的夜序只存在于本仓库，任何外部源都不会有**。
      2. **已经过时** —— 它缺 boffin、kazali、食人魔、大祭司、修验者、村民白痴、怨魂、召唤师、
         xaan、帽匠、公主、ojo、女妖、瘟疫医生、堤丰之主 全部 15 个 2023 年之后的官方角色。
         而且它的数值和本仓库的排序有 11 处相反（见下），本仓库的开头
         `DUSK → 堤丰之主 → 怨魂 → boffin → 失忆者 → 哲学家` 是现行官方顺序，
         townsquare 的数值是旧版。**导入它会把现在正确的顺序改坏。**

      **结论：A4 不要等 C4。** 夜序的事实来源就是 `night-order.json` 这个数组本身
      （已被 `characterPack.test.ts` 校验过：无未知 id、无重复）。A4 该做的是给现有数组
      按序号 ×10 赋值留出插入空隙，而不是去外部找一份不存在的权威数值。

      ⚠️ **附带发现，需要人看一眼**：拿 townsquare 数值和本仓库排序对比，有 11 处顺序相反。
      大概率是它数据旧，但也可能是本仓库真排错了，我没有第二个源可以判定：
      首夜 —— `amnesiac/philosopher`、`damsel/lunatic`、`marionette/sailor`；
      其他夜 —— `amnesiac/philosopher`、`pithag/poisoner`、`innkeeper/courtier`、
      `acrobat/snakecharmer`、`grandmother/barber`、`ravenkeeper/sage`、
      `farmer/professor`、`butler/undertaker`。
      已单列为 C12。
...
- [ ] **C12 · 复核 11 处夜序顺序分歧**（C4 附带发现）— 和 townsquare 旧数据对比出来的，
      大概率是对方数据旧，但需要拿现行官方夜序表核一遍。名单见 C4。
      纯核对，不改代码；确认没问题就关掉。
