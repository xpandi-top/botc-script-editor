...
- [ ] **A4 · 夜序数值化** — ✅ **已解除阻塞**（C4 查证结论：不存在外部权威数值，也不需要）。
      做法：以现有 `night-order.json` 数组为事实来源，按位置 ×10 赋值
      （`{ id, order }` 或 `CharacterEntry` 加 `firstNight`/`otherNight`），留出插入空隙。
      之后新增角色按数值排序插入，不再依赖「前位角色」锚点。
      奥德赛自己的夜序数值在 `assets/almanac/odyssey.zh.json` 里，可用来定新角色落在哪两个之间。
      要动 `buildEffectiveNightOrder`、夜序管理界面和 PDF 夜序表。
