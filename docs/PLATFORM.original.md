...
Keep React + Vite + MUI as the core. No rewrite.

**Capacitor** wraps the existing web build into native iOS/Android shells:
...
**PWA** as fallback: offline support, add-to-homescreen for Android/iOS browsers before Capacitor is integrated.
...
`☰` opens a bottom drawer replacing RightConsole (Player / Day / Game / Settings tabs).
...
Portrait: current Arena circular layout but without RightConsole pinned; swipe from right to open drawer.
...
- [ ] **P0-1** Add MUI breakpoint hooks to all layout components — no layout change, just wire up `isMobile`, `isTablet` booleans
...
- [ ] **P1-5** Mobile player card inline tag editor — tap-to-expand, no separate modal
...
- [ ] **P3-5** Tablet landscape: confirm current layout stable at all md/lg sizes; fix any edge cases
